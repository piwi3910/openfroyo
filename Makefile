.PHONY: build build-agent build-runner build-registry build-wasm clean test help

help:
	@echo "OpenFroyo Build Targets:"
	@echo "  make build          - Build froyo CLI, froyo-agent, froyo-runner, and froyo-registry"
	@echo "  make build-agent    - Build froyo-agent binary"
	@echo "  make build-registry - Build froyo-registry binary"
...
build: build-runner build-agent build-registry build-cli
...
build-registry:
	@echo "Building froyo-registry for multiple platforms..."
	@mkdir -p bin
	@echo "  → Linux amd64..."
	@CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o bin/froyo-registry-linux-amd64 ./cmd/froyo-registry
	@echo "  → Linux arm64..."
	@CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o bin/froyo-registry-linux-arm64 ./cmd/froyo-registry
	@echo "  → macOS amd64..."
	@CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o bin/froyo-registry-darwin-amd64 ./cmd/froyo-registry
	@echo "  → macOS arm64..."
	@CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o bin/froyo-registry-darwin-arm64 ./cmd/froyo-registry
	@echo "  → Windows amd64..."
	@CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o bin/froyo-registry-windows-amd64.exe ./cmd/froyo-registry
	@echo "✓ Built froyo-registry for 5 platforms"
